    ...
    public class MUMmerP1TestCases
    {
            ...
            LISParameters LISActionType, bool isMultiSequenceSearchFile)
        {
            ...
            else
            {
                ...
                string referenceAlphabet = _utilityObj._xmlUtil.GetTextValue(nodeName,
                ...
                referenceAlphabet = _utilityObj._xmlUtil.GetTextValue(nodeName,
            ...
            AdditionalParameters addParam)
        {
            ...
            switch (addParam)
            {
                ...
                case AdditionalParameters.PerformSimilarityMatrixChange:
                    ...
                    mum.GapOpenCost =
                        int.Parse(_utilityObj._xmlUtil.GetTextValue(nodeName, Constants.GapOpenCostNode),
            ...
            string[] expectedSequences =
                _utilityObj._xmlUtil.GetTextValues(nodeName, Constants.ExpectedSequencesNode);
        ...
        bool ValidateEdges(SequenceSuffixTree suffixTree, string nodeName)
        {
            ...
            for (int i = 0; i < expectedEdgeList.Count; i++)
            {
                ...
                {
                    Console.WriteLine(string.Format((IFormatProvider)null,
                        "MUMmer P1 : Edges not matching at index '{0}'", i.ToString((IFormatProvider)null)));
                    ApplicationLog.WriteLine(string.Format((IFormatProvider)null,
                        "MUMmer P1 : Edges not matching at index '{0}'", i.ToString((IFormatProvider)null)));
                    return false;
                }
        ...
        void ValidateUniqueMatchesForAllParameters(IList<MaxUniqueMatch> matches, string nodeName,
        ...
        /// <param name="LISActionType">Unique Match/Sub level LIS/LIS</param>
        /// <returns>True, if successfully validated</returns>
        bool ValidateUniqueMatches(IList<MaxUniqueMatch> matches, string nodeName,
            LISParameters LISActionType)
        {
            ...
            Console.WriteLine(string.Format((IFormatProvider)null, "MUMmer P1 : Total Matches Found is : '{0}'",
                matches.Count.ToString((IFormatProvider)null)));
            ApplicationLog.WriteLine(string.Format((IFormatProvider)null, "MUMmer P1 : Total Matches Found is : '{0}'",
                matches.Count.ToString((IFormatProvider)null)));
        ...
        void ValidateMUMsGeneralTestCases(string nodeName, bool isFilePath, bool isAfterLIS, bool isLIS)
        {
            ISequence referenceSeq = null;
            ISequence querySeq = null;
            IList<ISequence> querySeqs = null;
        ...
        {
            string[] firstSeqOrder = _utilityObj._xmlUtil.GetTextValue(
                nodeName, Constants.LisFirstSequenceMumOrderNode).Split(',');
            string[] firstSeqStart = _utilityObj._xmlUtil.GetTextValue(
                nodeName, Constants.LisFirstSequenceStartNode).Split(',');
            string[] length = _utilityObj._xmlUtil.GetTextValue(nodeName,
                Constants.LisLengthNode).Split(',');
            string[] secondSeqOrder = _utilityObj._xmlUtil.GetTextValue(nodeName,
                Constants.LisSecondSequenceMumOrderNode).Split(',');
            string[] secondSeqStart = _utilityObj._xmlUtil.GetTextValue(nodeName,
                Constants.LisSecondSequenceStartNode).Split(',');
            ...
            foreach (var mum in result)
            {
                ...
                for (int i = 0; i < mums.Count; i++)
                {
                    if (0 != string.Compare(querySeq.ToString(), mums[i].Query.ToString())
                       || (0 != string.Compare(firstSeqOrder[i], mums[i].FirstSequenceMumOrder.ToString((IFormatProvider)null)))
                       || (0 != string.Compare(firstSeqStart[i], mums[i].FirstSequenceStart.ToString((IFormatProvider)null)))
                       || (0 != string.Compare(length[i], mums[i].Length.ToString((IFormatProvider)null)))
                       || (0 != string.Compare(secondSeqOrder[i], mums[i].SecondSequenceMumOrder.ToString((IFormatProvider)null)))
                       || (0 != string.Compare(secondSeqStart[i], mums[i].SecondSequenceStart.ToString((IFormatProvider)null))))
                    {
                        Console.WriteLine(string.Format((IFormatProvider)null,
                            "MUMmer P1 : There is no match at '{0}'", i.ToString((IFormatProvider)null)));
                        ApplicationLog.WriteLine(string.Format((IFormatProvider)null,
                            "MUMmer P1 : There is no match at '{0}'", i.ToString((IFormatProvider)null)));
                        return false;
                    }
