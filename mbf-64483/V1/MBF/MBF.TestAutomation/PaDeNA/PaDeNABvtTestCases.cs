...
namespace MBF.TestAutomation.Algorithms.Assembly.PaDeNA
{
    ...
    public class PaDeNABvtTestCases
    {
        ...
        {
            PaDeNABvtTest testObj = new PaDeNA.PaDeNABvtTest();
        ...
        {
            PaDeNABvtTest testObj = new PaDeNABvtTest();
    ...
    internal class PaDeNABvtTest : ParallelDeNovoAssembler
    {
        ...
        {
            string nodesSequence = Utility._xmlUtil.GetTextValue(nodeName, Constants.NodesSequenceNode);
        ...
        internal void ValidatePaDeNADetectErrorNodes(string nodeName)
        {
            ...
            string kmerLength = Utility._xmlUtil.GetTextValue(nodeName,
                Constants.KmerLengthNode);
            string danglingSequence = Utility._xmlUtil.GetTextValue(nodeName,
        ...
        internal void ValidateSimpleContigBuilderBuild(string nodeName)
        {
            ...
            string expectedContigsString =
                Utility._xmlUtil.GetTextValue(nodeName, Constants.ContigsNode);
            string[] expectedContigs = expectedContigsString.Split(',');
        ...
        internal void AddLibraryInformation(string nodeName)
        {
            ...
            string mean = Utility._xmlUtil.GetTextValue(nodeName, Constants.Mean);
            ...
            CloneLibrary.Instance.AddLibrary(libraray,
                float.Parse(mean), float.Parse(StdDeviation));
            ...
            for (int index = 0; index < pairedreads.Count; index++)
            {
                Assert.IsTrue(forwardReadsNode.Contains(pairedreads[index].GetForwardRead(sequenceReads).ToString()));
                Assert.IsTrue(backwardReadsNode.Contains(pairedreads[index].GetReverseRead(sequenceReads).ToString()));
                Assert.IsTrue(deviationNode.Contains(pairedreads[index].StandardDeviationOfLibrary.ToString()));
                Assert.IsTrue(expectedLibrary.Contains(pairedreads[index].Library.ToString()));
                Assert.IsTrue(expectedMean.Contains(pairedreads[index].MeanLengthOfLibrary.ToString()));
            }
        ...
        internal void ValidateScaffoldPath(string nodeName)
        {
            ...
            Assert.AreEqual(expectedScaffoldPathCount, paths.Count.ToString());
            ...
            bool IsScaffold, bool EnableLowerContigRemoval, bool AllowErosion)
        {
            ...
            string mean = Utility._xmlUtil.GetTextValue(nodeName,
                Constants.Mean);
            string erosionThreshold = Utility._xmlUtil.GetTextValue(nodeName,
                Constants.ErosionNode);
            string lowCCThreshold = Utility._xmlUtil.GetTextValue(nodeName,
            ...
            if (EnableLowerContigRemoval)
            {
                ...
                denovoObj.ContigCoverageThreshold = double.Parse(lowCCThreshold);
            }

            if (AllowErosion)
            {
                denovoObj.AllowErosion = AllowErosion;
                denovoObj.ErosionThreshold = Int32.Parse(erosionThreshold);
            }
        ...
        /// </summary>
        /// <param name="nodeName">xml node name used for different testcases</param>
        IList<ISequence> SortContigsData(IList<ISequence> contigsList)
        {
        ...
        IList<ValidMatePair> SortPairedReads(
