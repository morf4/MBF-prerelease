...
namespace MBF.Algorithms.Assembly.PaDeNA
{
    ...
    public class RedundantPathsPurger : IGraphErrorPurger
    {
        ...
        /// </summary>
        private int _pathLengthThreshold;
        ...
        {
            get { return _pathLengthThreshold; }
            set { _pathLengthThreshold = value; }
        }
        #endregion
        ...
        public void RemoveErroneousNodes(DeBruijnGraph graph, DeBruijnPathList nodesList)
        {
            ...
            Parallel.ForEach(deleteNodes, node =>
            {
                foreach (DeBruijnNode extension in
                    node.LeftExtensionNodes.Keys.Union(node.RightExtensionNodes.Keys))
                {
            ...
            _graph.RemoveNodes(deleteNodes);
        }
        ...
        private static int GetBestPath(DeBruijnPathList divergingPaths)
        {
            ...
            {
                int sum = divergingPaths.Paths[i].PathNodes.Sum(n => n.KmerCount);
                if (sum > max)
                {
                    max = sum;
                    maxIndex = i;
        ...
        private void TraceDivergingExtensionPaths(
            ...
            Dictionary<DeBruijnNode, DeBruijnEdge> divergingNodes,
            ...
            List<DeBruijnPathList> redundantPaths)
        {
            List<PathWithOrientation> divergingPaths = new List<PathWithOrientation>(
                divergingNodes.Select(n =>
                    new PathWithOrientation(startNode, n.Key, n.Value.IsSameOrientation)));
            ...
            while (divergingPathLengh <= _pathLengthThreshold)
            {
                ...
                if (divergingPaths.Any(p => ((isForwardExtension ^ p.IsSameOrientation) ?
                      p.Nodes.Last().LeftExtensionNodes : p.Nodes.Last().RightExtensionNodes).Count != 1))
                {
                    return;
                }
                ...
                foreach (PathWithOrientation path in divergingPaths)
                {
                    ...
                    Dictionary<DeBruijnNode, DeBruijnEdge> extensions
                        = (isForwardExtension ^ path.IsSameOrientation) ? endNode.LeftExtensionNodes : endNode.RightExtensionNodes;

                    KeyValuePair<DeBruijnNode, DeBruijnEdge> nextNode = extensions.First();
                    ...
                    else
                    {
                        ...
                        path.IsSameOrientation = !(path.IsSameOrientation ^ nextNode.Value.IsSameOrientation);
