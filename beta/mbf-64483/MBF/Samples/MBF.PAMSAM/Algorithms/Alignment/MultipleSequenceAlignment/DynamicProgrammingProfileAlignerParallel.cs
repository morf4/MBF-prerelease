...
namespace MBF.Algorithms.Alignment.MultipleSequenceAlignment
{
    ...
    abstract public class DynamicProgrammingProfileAlignerParallel : IProfileAligner
    {
        ...
        public DynamicProgrammingProfileAlignerParallel()
            : this(new DiagonalSimilarityMatrix(2, -2, MoleculeType.DNA),
        ...
        */
        ...
        {
            // changed to profile-profile function
            //int diagScore = _FScore[col - 1, row - 1] + _similarityMatrix[_a[col - 1], _b[row - 1]];
            //float diagScore = _FScore[col - 1, row - 1] + _profileProfileScoreFunction(_similarityMatrix, _profileAlignmentA.ProfilesMatrix[_a[col - 1]], _profileAlignmentB.ProfilesMatrix[_b[row - 1]]);
        ...
        protected float SetCellValuesAffine(int col, int row)
        {
            ...
            */
            ...
            //M += _profileProfileScoreFunction(_similarityMatrix, _profileAlignmentA.ProfilesMatrix[_a[col - 1]], _profileAlignmentB.ProfilesMatrix[_b[row - 1]]);
        ...
        {
            Sequence result = new Sequence(seq.Alphabet);
            int x = 0, n;
        ...
        /*
        /// <summary>
        /// Writes matrix data in very detailed form.  Useful for debug and test in small cases.
        /// </summary>
        protected void DumpAffine()
        {
            int col, row;
            // Want header line
            row = 0;
            for (col = 0; col < _nCols; col++)
            {
                if (col == 0)
                {
                    ApplicationLog.Write("{0,2}      ", col);
                }
                else
                {
                    int[] tmp = new int[1];
                    tmp[0] = _a[col - 1];
                    similarityMatrix.ToByteArray(sequenceA[i].Symbol.ToString())
                    char c = _similarityMatrix.ToString(tmp)[0];
                    ApplicationLog.Write("{0,2} {1}           ", col, c);
                }
            }
            ApplicationLog.WriteLine("");
            // Then bc for row 0
            for (col = 0; col < _nCols; col++)
            {
                float Iy = Math.Max(_Iy[col, row], -99);  // BC values are -infinity, messes up the output
                if (col == 0)
                {
                    // write bc, M and Ix
                    ApplicationLog.Write("{0,3} {1,3} ", _M[col, row], Iy);
                }
                else
                {
                    ApplicationLog.Write("{0,4} {1,4}      ", _M[col, row], Iy);
                }
            }
            ApplicationLog.WriteLine("");
            ApplicationLog.WriteLine("");

            // For each row, start with col 0, which is header info as in first col above.
            // Then write M Iy and S, next line Ix and source
            for (row = 1; row < _nRows; row++)
            {
                for (col = 0; col < _nCols; col++)
                {
                    if (col == 0)
                    {
                        ...
                        tmp[0] = _b[row - 1];
                        char c = _similarityMatrix.ToString(tmp)[0];
                        ApplicationLog.Write("{0,2} {1}    ", row, c);
                    }
                    else
                    {
                        ApplicationLog.Write("{0,4} {1,4} {2,4} ", _M[col, row], _Iy[col, row], _similarityMatrix[_a[col - 1], _b[row - 1]]);
                ...
                for (col = 0; col < _nCols; col++)
                {
                    ...
                    {
                        // write bc, M and Ix
                        float Ix = Math.Max(_Ix[col, row], -99);  // BC values are -infinity, messes up the output

                        ApplicationLog.Write("{0,3} {1,3} ", _M[col, row], Ix);
                    }
                    else
                    {
                        ApplicationLog.Write("{0,4} {1,4}      ", _Ix[col, row], _FSource[col, row]);
                    }
                }
                ApplicationLog.WriteLine("");
                ApplicationLog.WriteLine("");
