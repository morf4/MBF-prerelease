...
namespace MBF.Tests
{
    ...
    public class BAMTests
    {
        ...
        {
            string filePath = @"TestUtils\BAM\SeqAlignment.bam";
            string outputfilePath = "BAMTests1.bam";
            BAMParser parser = null;
            try
            {
                parser = new BAMParser();
                ...
                formatter.Format(alignmentMap, "BAMTests.bam");

                Assert.IsTrue(File.Exists("BAMTests.bam.bai"));
                ...
                alignmentMap = parser.ParseRange("BAMTests.bam", new SequenceRange("chr20", 0, int.MaxValue));

                Assert.IsTrue(alignmentMap != null);
                Assert.AreEqual(alignmentMap.Header.GetReferenceSequences().Count, 1);
                Assert.AreEqual(alignmentMap.QuerySequences.Count, 2);

                alignmentMap = parser.ParseRange("BAMTests.bam", new SequenceRange("chr20", 0, 28833));
