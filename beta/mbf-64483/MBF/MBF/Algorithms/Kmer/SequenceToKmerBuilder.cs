...
namespace MBF.Algorithms.Kmer
{
    ...
    public class SequenceToKmerBuilder : IKmerBuilder
    {
        ...
        public static Dictionary<string, KmerDataGraphNodePair> BuildPaDeNAKmerDictionary(IList<ISequence> sequences, int kmerLength)
        {
            ...
            var kmerDictionaries = 
                sequences.AsParallel().Select((sequence, ndx) => BuildPaDeNAKmerDictionary(ndx, sequence, kmerLength)).ToList();
        ...
        /// Constructs KmersOfSequence for sequence and associated k-mers.
        /// </summary>
        /// <param name="sequenceIndex">Index of input sequence</param>
        /// <param name="sequence">Input sequence</param>
        /// <param name="kmerLength">k-mer length</param>
        /// <returns>KmersOfSequence constructed from sequence and associated k-mers</returns>
        public static Dictionary<string, KmerDataGraphNodePair> BuildPaDeNAKmerDictionary(int sequenceIndex, ISequence sequence, int kmerLength)
        {
            if (sequence == null)
            {
                throw new ArgumentNullException(Resource.SequenceCannotBeNull);
            ...
            for (int i = 0; i <= sequence.Count - kmerLength; ++i)
            {
                ...
                kmerDict.TryGetValue(kmer, out kmerData);
                if (kmerData == null)
                    kmerDict.Add(kmer, new KmerDataGraphNodePair(new KmerData(sequenceIndex, i, isNormalOrientation), isNormalOrientation));
                else
        ...
        /// <returns>Merged Dictionary</returns>
        private static Dictionary<string, KmerDataGraphNodePair> MergeKmerDictionaries(List<Dictionary<string, KmerDataGraphNodePair>> kmerDicts, int start, int end)
        {
