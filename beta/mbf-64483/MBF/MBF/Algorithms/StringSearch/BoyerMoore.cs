...
namespace MBF.Algorithms.StringSearch
{
    /// <summary>
    ...
    public class BoyerMoore : IPatternFinder
    {
        ...
        public IList<int> FindMatch(ISequence sequence, string searchPattern)
        {
            ...
            int patternIndex = 0, patternfoundAt = 0, foundAt, fullStringLength, searchStringLength;
            ...
            fullStringLength = sequence.Count;
            ...
            while (foundAt <= fullStringLength - searchStringLength)
            {
                ...
                for (int index = subPattern.Length - 1; index >= 0; index--)
                {
                    ...
                    {
                        if ((containsWildcard[patternIndex - 1] == sequence[patternfoundAt + foundAt].Symbol))
                        {
                            do
                            {
                                --patternfoundAt;
                            }
                            while (containsWildcard[patternIndex - 1] == sequence[patternfoundAt + foundAt].Symbol
                ...
                {
                    foundAt += Math.Max(goodSuffixes[patternIndex], badCharacters[sequence[patternfoundAt + foundAt].Symbol] - searchStringLength + 1 + patternIndex);
                }
            }
                ...
                out int patternfoundAt)
        {
            ...
            {
                if (searchPattern[patternIndex] != sequence[patternfoundAt + foundAt].Symbol)
                {
                    break;
                }
            }
