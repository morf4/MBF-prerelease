...
namespace MBF.Web.Blast
{
    ...
    public class BioHPCBlastHandler : IBlastServiceHandler, IDisposable
    {
        ...
        /// </summary>
        private BioHPCClient _blastClient;
        ...
        public ServiceRequestInformation GetRequestStatus(string requestIdentifier)
        {
            ...
            status.StatusInformation = this._blastClient.GetJobInfo(jobidcntrl.Jobid, jobidcntrl.Cntrl);
        ...
        public string SubmitRequest(IList<ISequence> sequences, BlastParameters parameters)
        {
            ...
            try
            {
                string[] outtab = _blastClient.CreateJob(appID, jobname, "1", Configuration.EmailAddress, Configuration.Password, "Auto");
        ...
        public bool CancelRequest(string requestIdentifier)
        {
            ...
            try
            {
                return _blastClient.CancelJob(jobidcntrl.Jobid, jobidcntrl.Cntrl).IndexOf(MsgError, StringComparison.Ordinal) == -1;
            }
            catch
            {
                throw new Exception(Resources.BIOHPCSERVERABSENT);
        ...
        public string[] GetServiceMetadata(string kind)
        {
            ...
            switch (kind)
            {
                case MetadataDatabasesDna:
                    data = _blastClient.GetBlastDatabases(Configuration.EmailAddress, Configuration.Password, "n");
                    break;
                case MetadataDatabasesProt:
                    data = _blastClient.GetBlastDatabases(Configuration.EmailAddress, Configuration.Password, "p");
                    break;
                case MetadataFilter:
                    data = Enum.GetNames(typeof(LowCompFilter));
                    break;
                case MetadataMatrices:
                    data = Enum.GetNames(typeof(Matrix));
                    break;
                ...
                case MetadataFormats:
                    ...
                    break;
                default: data = _blastClient.GetBlastDatabases(Configuration.EmailAddress, Configuration.Password, "n");
                    break;
            }
