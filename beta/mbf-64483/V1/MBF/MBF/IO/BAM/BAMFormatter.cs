...
namespace MBF.IO.BAM
{
    ...
    public class BAMFormatter : ISequenceAlignmentFormatter
    {
        ...
        {
            CreateBAMIndexFile(bamFileName, bamFileName + Resource.BAM_INDEXFILEEXTENSION);
        }
        ...
        private void WriteUncompressed(SequenceAlignmentMap sequenceAlignmentMap, Stream writer, bool createSortedFile)
        {
            ...
            if (createSortedFile)
            {
                if (sequenceAlignmentMap.QuerySequences is IVirtualAlignedSequenceList<SAMAlignedSequence>)
                {
                    BAMSort sorter = new BAMSort(sequenceAlignmentMap, SortType);
                    ...
                    foreach (BAMSortedIndex sortedGroup in sortedGroups)
                    {
                        ...
                        {
                            SAMAlignedSequence alignedSeq = sequenceAlignmentMap.QuerySequences[index];
                ...
                {
                    if (SortType == BAMSortByFields.ChromosomeCoordinates)
                    {
