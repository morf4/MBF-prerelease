...
namespace MBF.TestAutomation.Web.EbiBlast
{
    ...
    public class EbiBlastP1TestCases
    {
        ...
        {
            EbiWuBlastHandler ebiHandler = new EbiWuBlastHandler();
        ...
        /// </summary>
        static void GeneralMethodToValidateEbiBlastParameters(
            string nodeName, ParameterGroup parameters)
        {
            if (_IsWebServiceAvailable)
            {
                ...
                string email = Utility._xmlUtil.GetTextValue(
                    nodeName, Constants.EmailAdress);
                string optionalStrandParameter = Utility._xmlUtil.GetTextValue(
                    nodeName, Constants.Emailparameter);
                string strandValue = Utility._xmlUtil.GetTextValue(
                    nodeName, Constants.EmailAdress);
                string optionalSensitivityParameter = Utility._xmlUtil.GetTextValue(
                    nodeName, Constants.Emailparameter);
                string sensitivityValue = Utility._xmlUtil.GetTextValue(
                    nodeName, Constants.EmailAdress);
        ...
        /// </summary>
        static void ValidateSubmitSearchMethod(
            string nodeName, ParameterGroup parameters)
        {
            if (_IsWebServiceAvailable)
            {
        ...
        static bool ValidateWebServiceConnection()
        {
            ...
            catch (Exception ex)
            {
                Console.WriteLine(string.Format(
                    "Ebi Blast P1 : Connection not successful with error '{0}'", ex.Message));
                ApplicationLog.WriteLine(string.Format(
                    "Ebi Blast P1 : Connection not successful with error '{0}'", ex.Message));
                return false;
            }

            Console.WriteLine("Ebi Blast P1 : Connection Successful");
            return true;
        }
