...
from MBF.Algorithms.Kmer import *
...
def ConcatenateSequences():
    ...
    while "yY".find(again[0]) != -1:
        ...
        for sequence in firstSet:
            firstSetConcatenated += sequence.ToString()
        
        # Concatenate all sequences present in the second file    
        for sequence in secondSet:
            secondSetConcatenated += sequence.ToString()
...
def StripNonAlphabets():
    ...
    while "yY".find(again[0]) != -1:
        ...
        for sequence in firstSet:
            ...
            # If sequence contained non-alphbetic characters
            if sequenceChanged == 1:
                sequence.IsReadOnly = 0;
                
                # Remove the older alphabets and add the new cleaned-up alphabets
                sequence.RemoveRange(0, sequence.Count)
                sequence.InsertRange(0, newSequence)
                sequence.IsReadOnly = 1;
    ...
    while "yY".find(again[0]) != -1:
        ...
        for sequence in firstSet:
            ...
            if  polyATailLength > 5:
                sequence.IsReadOnly = 0;
                sequence.RemoveRange(endIndex+1, polyATailLength)
...
def DiffSeq():
    ...
    while "yY".find(again[0]) != -1:
        ...
        # Get all the matches. 
        nodes = WordMatch.BuildMatchTable(c_sharp_list, first_Sequence, second_Sequence, 2);
