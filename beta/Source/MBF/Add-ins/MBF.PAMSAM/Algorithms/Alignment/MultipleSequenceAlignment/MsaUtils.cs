    ...
    public static class MsaUtils
    {
                                                ...
                                                int gapOpenPenalty, int gapExtensionPenalty)
        {
            ...
            for (int i = 0; i < sequenceA.Count; ++i)
            {
                ...
                byte a = similarityMatrix.ToByte(sequenceA[i].Symbol);
                byte b = similarityMatrix.ToByte(sequenceB[i].Symbol);
                result += similarityMatrix[a, b];
            }
        ...
        public static ISequence UnAlign(ISequence alignedSequence)
        {
            ...
            {
                if (!alignedSequence[i].IsGap)
                {
                    unalignedSequence.Add(alignedSequence[i]);
                }
            }
            return unalignedSequence;
        }
        ...
        public static List<int> CalculateOffset(ISequence sequence, ISequence sequenceRef)
        {
            ...
            {
                while (indexA < sequence.Count && sequence[indexA].IsGap)
                {
                    result.Add(-1);
                    ++indexA;
                }
                while (indexRef < sequenceRef.Count && sequenceRef[indexRef].IsGap)
                {
                    ++indexRef;
                }
                result.Add(indexRef);
        ...
        public static float CalculateAlignmentScoreQ(IList<ISequence> sequences, IList<ISequence> sequencesRef)
        {
            ...
            int sequenceLength = sequences[0].Count;
            int sequenceLengthRef = sequencesRef[0].Count;
                ...
                for (int j = i + 1; j < sequences.Count; ++j)
                {
                    for (int k = 0; k < sequenceLength; ++k)
                    {
                        if (offsets[i][k] == -1 && offsets[j][k] != -1)
                        {
                            try
                            {
                                if (sequencesRef[i][offsets[j][k]].IsGap)
                                {
                        ...
                        else if (offsets[j][k] == -1 && offsets[i][k] != -1)
                        {
                            try
                            {
                                if (sequencesRef[j][offsets[i][k]].IsGap)
                                {
                                    ++numberOfCorrectResidues;
                                }
                            }
        ...
        public static void SetProfileItemSets(MoleculeType moleculeType)
        {
            ...
            switch (moleculeType)
            {
                case (MoleculeType.DNA):
                    ...
                    basics = new ISequenceItem[2] { Alphabets.DNA.A, Alphabets.DNA.C };
                    ambiguousCharacterMap.Add(Alphabets.DNA.AC, new List<ISequenceItem>(basics));
                    basics = new ISequenceItem[2] { Alphabets.DNA.G, Alphabets.DNA.C };
                    ambiguousCharacterMap.Add(Alphabets.DNA.GC, new List<ISequenceItem>(basics));
                    basics = new ISequenceItem[2] { Alphabets.DNA.A, Alphabets.DNA.T };
                    ambiguousCharacterMap.Add(Alphabets.DNA.AT, new List<ISequenceItem>(basics));
                    basics = new ISequenceItem[2] { Alphabets.DNA.A, Alphabets.DNA.G };
                    ambiguousCharacterMap.Add(Alphabets.DNA.GA, new List<ISequenceItem>(basics));
                    basics = new ISequenceItem[2] { Alphabets.DNA.C, Alphabets.DNA.T };
                    ambiguousCharacterMap.Add(Alphabets.DNA.TC, new List<ISequenceItem>(basics));
                    basics = new ISequenceItem[2] { Alphabets.DNA.G, Alphabets.DNA.T };
                    ambiguousCharacterMap.Add(Alphabets.DNA.GT, new List<ISequenceItem>(basics));
                    basics = new ISequenceItem[3] { Alphabets.DNA.C, Alphabets.DNA.G, Alphabets.DNA.T };
                    ambiguousCharacterMap.Add(Alphabets.DNA.GTC, new List<ISequenceItem>(basics));
                    basics = new ISequenceItem[3] { Alphabets.DNA.A, Alphabets.DNA.C, Alphabets.DNA.G };
                    ambiguousCharacterMap.Add(Alphabets.DNA.GCA, new List<ISequenceItem>(basics));
                    basics = new ISequenceItem[3] { Alphabets.DNA.A, Alphabets.DNA.C, Alphabets.DNA.T };
                    ambiguousCharacterMap.Add(Alphabets.DNA.ACT, new List<ISequenceItem>(basics));
                    basics = new ISequenceItem[3] { Alphabets.DNA.A, Alphabets.DNA.G, Alphabets.DNA.T };
                    ambiguousCharacterMap.Add(Alphabets.DNA.GAT, new List<ISequenceItem>(basics));
                    basics = new ISequenceItem[4] { Alphabets.DNA.A, Alphabets.DNA.C, Alphabets.DNA.G, Alphabets.DNA.T };
                ...
                case (MoleculeType.Protein):
                    ...
                    basics = new ISequenceItem[2] { Alphabets.Protein.Asn, Alphabets.Protein.Asp };
                    ambiguousCharacterMap.Add(Alphabets.Protein.Asx, new List<ISequenceItem>(basics));
                    basics = new ISequenceItem[2] { Alphabets.Protein.Leu, Alphabets.Protein.Ile };
                    ambiguousCharacterMap.Add(Alphabets.Protein.Xle, new List<ISequenceItem>(basics));
                    basics = new ISequenceItem[2] { Alphabets.Protein.Gln, Alphabets.Protein.Gln };
                ...
                case (MoleculeType.RNA):
                    ...
                    basics = new ISequenceItem[2] { Alphabets.RNA.A, Alphabets.RNA.C };
                    ambiguousCharacterMap.Add(Alphabets.RNA.AC, new List<ISequenceItem>(basics));
                    basics = new ISequenceItem[2] { Alphabets.RNA.G, Alphabets.RNA.C };
                    ambiguousCharacterMap.Add(Alphabets.RNA.GC, new List<ISequenceItem>(basics));
                    basics = new ISequenceItem[2] { Alphabets.RNA.A, Alphabets.RNA.U };
                    ambiguousCharacterMap.Add(Alphabets.RNA.AU, new List<ISequenceItem>(basics));
                    basics = new ISequenceItem[2] { Alphabets.RNA.A, Alphabets.RNA.G };
                    ambiguousCharacterMap.Add(Alphabets.RNA.GA, new List<ISequenceItem>(basics));
                    basics = new ISequenceItem[2] { Alphabets.RNA.C, Alphabets.RNA.U };
                    ambiguousCharacterMap.Add(Alphabets.RNA.UC, new List<ISequenceItem>(basics));
                    basics = new ISequenceItem[2] { Alphabets.RNA.G, Alphabets.RNA.U };
                    ambiguousCharacterMap.Add(Alphabets.RNA.GU, new List<ISequenceItem>(basics));
                    basics = new ISequenceItem[3] { Alphabets.RNA.C, Alphabets.RNA.G, Alphabets.RNA.U };
                    ambiguousCharacterMap.Add(Alphabets.RNA.GUC, new List<ISequenceItem>(basics));
                    basics = new ISequenceItem[3] { Alphabets.RNA.A, Alphabets.RNA.C, Alphabets.RNA.G };
                    ambiguousCharacterMap.Add(Alphabets.RNA.GCA, new List<ISequenceItem>(basics));
                    basics = new ISequenceItem[3] { Alphabets.RNA.A, Alphabets.RNA.C, Alphabets.RNA.U };
                    ambiguousCharacterMap.Add(Alphabets.RNA.ACU, new List<ISequenceItem>(basics));
                    basics = new ISequenceItem[3] { Alphabets.RNA.A, Alphabets.RNA.G, Alphabets.RNA.U };
                    ambiguousCharacterMap.Add(Alphabets.RNA.GAU, new List<ISequenceItem>(basics));
                    basics = new ISequenceItem[4] { Alphabets.RNA.A, Alphabets.RNA.C, Alphabets.RNA.G, Alphabets.RNA.U };
