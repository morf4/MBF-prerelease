    ...
    public class SegmentedSequence : ISequence
    {
        ...
        public IEncoding Encoding
        {
            get
            {
                ...
                {
                    encoding = _sequences[0].Encoding;
                }

                for (int i = 1; i < _sequences.Count; i++)
                {
                    if (encoding != _sequences[i].Encoding)
                    {
                        encoding = null;
                    }
                }
        ...
        public ISequenceItem this[int index]
        {
            get
            {
                ...
                ISequenceItem outputItem = seq[localIndex];
                ...
                else
                {
                    ...
                    {
                       return GetSymbolSafeISequenceItem(seq.Encoding, outputItem);
                    }
                }
            }
            set
        ...
        int IList<byte>.IndexOf(byte item)
        {
            ...
            // Get the sequence item for the given byte value
            if (UseEncoding)
            {
        ...
        byte IList<byte>.this[int index]
        {
            get
            {
                ...
                if (UseEncoding) // if all sequences are encoded and having same encoding
                {
                    int localIndex = GetSequenceLocalPosition(index);
                    IList<byte> seq = GetSequenceByPosition(index);
        ...
        void ICollection<byte>.Add(byte item)
        {
            ...
            if (UseEncoding)
            {
                seqItem = Encoding.LookupByValue(item);
            }
            else
            {
                seqItem = this.Alphabet.LookupByValue(item);
            }
            if (seqItem == null)
                throw new ArgumentException(string.Format(CultureInfo.CurrentCulture, Properties.Resource.InvalidByteValue, item));
        ...
        private ISequenceItem GetSymbolSafeISequenceItem(IEncoding encoding, ISequenceItem item)
        {
            return GetMapToAlphabet(encoding).Convert(item);
        }
        ...
        private EncodingMap GetMapToAlphabet(IEncoding encoding)
        {
            if (_mapToAlphabet == null || _mapToAlphabet.Encoding != encoding)
            {
