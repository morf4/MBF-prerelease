...
namespace MBF
{
    ...
    public class SparseSequence : ISequence
    {
            ...
            : this(alphabet)
        {
            ...
            _sparseSeqItems.Add(index, seqItem);
        ...
        {
            ISequenceItem sequenceItem = this.Alphabet.LookupByValue(item);

            if (sequenceItem == null)
                return -1;

            IList<byte> byteSeq = this as IList<byte>;
            for (int i = 0; i < Count; i++)
            {
                if (byteSeq[i] == item)
                    return i;
        ...
        byte IList<byte>.this[int index]
        {
            ...
            set
            {
                ...
                // Check whether the byte is known by alphabet of this sequence or not.
                ISequenceItem sequenceItem = this.Alphabet.LookupByValue(value);
                if (sequenceItem == null)
        ...
        void ICollection<byte>.Add(byte item)
        {
            ...
            ISequenceItem seqItem = Alphabet.LookupByValue(item);

            if (seqItem == null)
                throw new ArgumentException(string.Format(CultureInfo.CurrentCulture, Properties.Resource.InvalidByteValue, item));

            _sparseSeqItems.Add(Count, seqItem);
            _statistics.Add(seqItem);
        ...
        void ICollection<byte>.CopyTo(byte[] array, int arrayIndex)
        {
            ...
            foreach (byte seqByteValue in (this as IList<byte>))
            {
                array[arrayIndex++] = seqByteValue;
            }
        }
        ...
        bool ICollection<byte>.Remove(byte item)
        {
            ...
            int index = (this as IList<byte>).IndexOf(item);
            if (index >= 0)
            {
                RemoveRange(index, 1);
                return true;
