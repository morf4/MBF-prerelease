    ...
    public class QualitativeSequence : IQualitativeSequence, IVirtualSequence
    {
        ...
        public override string ToString()
        {
            if (VirtualQualitativeSequenceProvider != null)
            {
                ...
                {
                    buffer[i] = VirtualQualitativeSequenceProvider.GetItem(i);
                }
        ...
        public bool ContainsQualityScore(byte qualScore)
        {
            if (IsReadOnly)
            {
        ...
        {
            get { return VirtualQualitativeSequenceProvider != null; }
        }
        #endregion
        ...
        public ISequenceItem this[int index]
        {
            get
            {
                if (index >= 0 && index < Count)
                {
                    if (VirtualQualitativeSequenceProvider != null)
                    {
                        if (UseEncoding)
                        {
                            return Encoding.LookupBySymbol(VirtualQualitativeSequenceProvider.GetItem(index));
                        }
                        else
                        {
                            return VirtualQualitativeSequenceProvider.GetISequenceItem(index);
                        }
                    }
            ...
            set
            {
                ...
                if (VirtualQualitativeSequenceProvider != null)
                {
                    ...
                    VirtualQualitativeSequenceProvider[index] = (byte)value.Symbol;
                }
                else
                {
        ...
        public void CopyTo(ISequenceItem[] array, int arrayIndex)
        {
            ...
            // non-DV copy
            if (UseEncoding || VirtualQualitativeSequenceProvider == null)
            {
        ...
        int IList<byte>.IndexOf(byte item)
        {
            ...
            if (VirtualQualitativeSequenceProvider == null)
            {
                return (_sequence as IList<byte>).IndexOf(item);
            }
            else // DV index
            {
                ...
                return VirtualQualitativeSequenceProvider.IndexOf((byte)sequenceItem.Symbol);
            }
        }
        ...
        void IList<byte>.Insert(int index, byte item)
        {
            ...
            if (VirtualQualitativeSequenceProvider == null)
            {
                _sequence.Insert(index, sequenceItem);
            }
            else // DV insert
            {
                VirtualQualitativeSequenceProvider.Insert(index, sequenceItem);
            }

            // Use default quality score
            _scoreList.Insert(index, this.GetDefaultQualScore());
        }
        ...
        byte IList<byte>.this[int index]
        {
            get
            {
                if (index >= 0 && index < Count)
                {
                    if (VirtualQualitativeSequenceProvider != null)
                    {
                        if (UseEncoding)
                        {
                            return _sequence.Encoding.LookupBySymbol(VirtualQualitativeSequenceProvider.GetItem(index)).Value;
                        }
                        else
                        {
                    ...
                    {
                        return (_sequence as IList<byte>)[index];
                    }
                }
            ...
            set
            {
                ...
                if (VirtualQualitativeSequenceProvider == null)
                {
                    (_sequence as IList<byte>)[index] = value;
                }
                else // DV insert
                {
                    // Check whether the byte is known by alphabet of this sequence or not.
                    ISequenceItem sequenceItem = GetAlphabetISeqeunceItem(value);

                    if (sequenceItem == null)
                        throw new ArgumentException(string.Format(CultureInfo.CurrentCulture, Properties.Resource.InvalidByteValue, value));

                    VirtualQualitativeSequenceProvider[index] = sequenceItem.Value;
                }
        ...
        void ICollection<byte>.CopyTo(byte[] array, int arrayIndex)
        {
            ...
            foreach (byte seqByte in (_sequence as IList<byte>))
            {
