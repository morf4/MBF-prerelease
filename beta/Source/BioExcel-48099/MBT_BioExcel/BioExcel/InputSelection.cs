...
namespace BioExcel
{
    ...
    public class InputSelection
    {
                ...
                foreach (InputSequenceItem currentSequenceItem in sequenceItems)
                {
                    try
                    {
                        ...
                        if (rangesInCurrentSequenceItem.Count > 0)
                        {
                            ...
                            {
                                // Set the ID
                                SetSequenceID(sequenceForCurrentItem, currentSequenceItem.SequenceName);
                            }
        ...
        private void SetSequenceID(object targetSequence, string sequenceID)
        {
            if (targetSequence is Sequence)
            {
                (targetSequence as Sequence).ID = sequenceID;
                (targetSequence as Sequence).DisplayID = sequenceID;
            }
            else if (targetSequence is QualitativeSequence)
            {
                (targetSequence as QualitativeSequence).ID = sequenceID;
                (targetSequence as QualitativeSequence).DisplayID = sequenceID;
            }
        }
                ...
                params object[] argsForCallback)
        {
            ...
            if (argsForCallback[0] is MBF.IO.Fasta.FastaFormatter)
            {
                selectionDialog.IsMetadataVisible = false;
            }
            else if (argsForCallback[0] is MBF.IO.FastQ.FastQFormatter)
            {
            ...
            try
            {
                foreach (InputSequenceItem currentSequenceItem in sequenceItems)
                {
                    try
                    {
                        ...
                        //Parse metadata
                        if (formatterUsed is MBF.IO.FastQ.FastQFormatter)
                        {
                        ...
                        else if (formatterUsed is GenBankFormatter)
                        {
                            ...
                            if (rangesInCurrentSequenceItem.Count > 0 && sequenceForCurrentItem != null)
                            {
                                ...
                                {
                                    // Set the ID
                                    SetSequenceID(sequenceForCurrentItem, metadata.Locus.Name);
                                }
                            }
                        }
