...
namespace MBF.TestAutomation.Web.Blast
{
    ...
    public class BlastBvtTestCases
    {
        ...
        /// <summary>
        /// Validate ServiceRequestInformation GetObjectData.
        /// Input Data  :Valid search query, Database value and program value.
        ...
        [Priority(0)]
        [TestCategory("Priority0")]
        public void ValidateServiceRequestInformationGetObjectData()
        {
            // Gets the search query parameter and their values.
            string querySequence = _utilityObj._xmlUtil.GetTextValue(
                Constants.BlastDnaSequenceParametersNode, Constants.QuerySequency);
            ...
            object resultsObject = _TestCaseSimulator.Simulate(parameters).Result;
            ServiceRequestInformation reqInfo = new ServiceRequestInformation();
            reqInfo = resultsObject as ServiceRequestInformation;
            ...
            try
            {
                reqInfo.GetObjectData(null, context);
                Assert.Fail();
            }
            catch (ArgumentNullException)
            {
                reqInfo.GetObjectData(info, context);
                Assert.IsNotNull(reqInfo);
            }
                ...
                && reqInfo.Status != ServiceRequestStatus.Ready)
            {
                string error = ApplicationLog.WriteLine(string.Concat(
                    "Unexpected error ", reqInfo.Status));
                Assert.Fail(error);
                ApplicationLog.WriteLine(string.Concat("Unexpected error ",
                    reqInfo.Status));
            }
            else
            {
