...
namespace MBF.TestAutomation
{
    ...
    public class SegmentedSequenceP2TestCases
    {
        ...
        public void ValidateExceptionByModifyingReadOnlySegSequence()
        {
            ...
            string firstSequence = _utilityObj._xmlUtil.GetTextValue(
                Constants.ProteinSegSequenceForErrorValidationNode, Constants.FirstSequence);
            string secondSeq = _utilityObj._xmlUtil.GetTextValue(
        ...
        public void ValidateExceptionByAddingRnaSeqItemToDnaSegSequence()
        {
            ...
            {
                // Log to Nunit GUI.
                Console.WriteLine("Segmented Sequence P2: Segmented Sequence Exception was validated successfully");
            }
        ...
        public void InvalidateSegmentedSeqCtors()
        {
            ...
            SegmentedSequence segSeq = null;
            try
            {
                segSeq = new SegmentedSequence(null as ISequence);
                Assert.Fail();
            }
            catch (ArgumentNullException ex)
            {
            ...
            {
                if (segSeq != null)
                    ((IDisposable)segSeq).Dispose();
            }


            try
            {
                segSeq = new SegmentedSequence(null as ICollection<ISequence>);
                Assert.Fail();
            }
            catch (ArgumentNullException ex)
            {
            ...
            try
            {
                segSeq = new SegmentedSequence(nullSeqs);
                Assert.Fail();
            }
            catch (ArgumentNullException ex)
            {
            ...
            try
            {
                segSeq = new SegmentedSequence(seqList);
                Assert.Fail();
            }
            catch (ArgumentException ex)
            {
        ...
        {
            string firstInputSeq = _utilityObj._xmlUtil.GetTextValue(
        ...
        void ValidateDifferentAlphabetSegSequence(
            string nodeName, IAlphabet alphabet1, IAlphabet alphabet2)
        {
            string firstInputSeq = _utilityObj._xmlUtil.GetTextValue(
                nodeName, Constants.FirstSequence);
            string secondInputSeq = _utilityObj._xmlUtil.GetTextValue(
        ...
        /// </summary>
        /// <param name="nodeName" >Name of the error message node </param>
        SegmentedSequence CreateSegmentedSequence(
            string nodeName, IAlphabet alphabet1, IAlphabet alphabet2, bool readOnly)
        {
            string dnaInputSeq = _utilityObj._xmlUtil.GetTextValue(
                nodeName, Constants.FirstSequence);
            string rnaInputSeq = _utilityObj._xmlUtil.GetTextValue(
                nodeName, Constants.SecondSegSequence);
            List<ISequence> seqList = new List<ISequence>();
            SegmentedSequence segmentedSeq = null;
